...
#  google_upload.OrganizeGooleDrive()
#  google_upload.uploadFile(gUpLog, '')
#  google_upload.uploadFile(gUpImg, '')

  logger.debug('upload start')

  # カレントディレクトリの変更
  os.chdir('/home/pi/work/python/power-switch/plot')

  import os 
  from datetime import datetime
  #sdate =  datetime.fromtimestamp(os.path.getctime(gUpLog)).strftime("%Y%m%d") 
  sdate =  datetime.fromtimestamp(os.stat(gUpLog).st_atime).strftime("%Y%m%d")

  import google_upload 
  google_upload.OrganizeGooleDrive() 
  google_upload.uploadFile(gUpLog, sdate + '.txt')
  google_upload.uploadFile(gUpImg, sdate + '.png')
